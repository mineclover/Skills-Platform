...
> **상태**: 루프 설계 참고 매트릭스 v1.1 — 현재 Catalog 추천·설치 목록과 구분
...
현재 프로젝트의 최소 기본셋, 작업별 후보, 추천 제외 사유와 실제 선택 규칙은
[추천 스킬셋·정렬 가이드](./recommended-skillsets-guide.md)를 기준으로 합니다.
아래 루프·스킬 매핑과 게이트는 설계 자료이며, 이름이 표에 있다는 이유로 해당 스킬·도구가
설치되었거나 hook이 강제된다고 판단하지 않습니다. `recommended` 역할은 자동 선택을
뜻하지 않으며, provider별 원본·의존성 검토 후 default 또는 작업 overlay로 선택해야 합니다.
...
각 루프 유형에서 검토할 생태계별 후보를 정리한 설계 매핑입니다. 현재 원본의 역할과
실행 환경을 다시 확인해야 하며, 이 표는 전체 조합의 독립 실행 검증 결과가 아닙니다.
예를 들어 `re0-work`는 기존 교훈을 보존한 v0 재시작, `debloat`는 문장 밀도 정리,
`autobahn`은 위험 인접 요청의 범위 분리 역할이므로 일반 TDD·핫픽스의 기본 스킬로
자동 채택하지 않습니다.
...
4. **회귀 검증(Regression Gate)**: 변경한 동작에 맞는 검증부터 실행하고, 공유 계약·전달 경로에 영향을 주면 필요한 회귀 범위로 넓힙니다. 테스트 수는 현재 실행 결과를 기록하며 과거 고정 수치를 통과 근거로 사용하지 않습니다.

...
const { filterSkillSearchEntries } = require("@skills-platform/contracts/skill-search");
const { getSkillLineage, getSourceRevision, latestSkillsByArtifact, listRegistrySkills, listSkillLineages } = require("./registry");
const { getPreset, loadCatalog, saveCatalog, mutateCatalog } = require("./catalog-state");
...
async function searchSkills({ catalogRoot, registryRoot, query = "", tags = [], domains = [], providerId, reviewState, artifactType, invocationMode }) {
  ...
  const entries = context.lineages.map((lineage) => {
  ...
  return filterSkillSearchEntries(entries, { query, tags, domains, providerId, reviewState, artifactType, invocationMode });
}

...
All Vertical Spec documents MUST contain the following structured sections:
...
- `Contract Ref`: Bound `completion_contract.yaml` ID and obligation IDs.
- `Lifecycle State`: `IN_PROGRESS`, `VERIFIED`, `REOPENED`, `CLOSED`.

### 2. Architecture & Topology Alignment
- `Detected Topology`: `feature-first` | `layer-first` | `framework-idiomatic` | `monorepo-split`.
- `Architecture Pattern`: `MVVM` | `MVP` | `Clean-Hexagonal` | `Flux` | `Custom`.
- `Owned Target Files`: Exact files that this vertical topic is authorized to modify following project topology.
...
### 3. Concrete Behavioral & Architectural Invariants
- `Pre-conditions`: State required prior to execution.
- `Post-conditions`: Deterministic expected outcomes after execution.
- `Strict Invariants`: Dependency direction rules (e.g. ViewModel UI-isolation, domain purity), type safety, latency ceilings.

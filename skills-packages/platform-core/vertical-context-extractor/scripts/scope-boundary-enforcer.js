...
function loadTopicSpec(payload = {}, projectRoot = process.cwd()) {
  // 1. Direct spec in payload or nested toolCall
  const sources = [
    payload,
  ...
  for (const src of sources) {
    ...
    if (obj && typeof obj === "object") {
      ...
      if (obj.topic_spec && typeof obj.topic_spec === "object") return obj.topic_spec;
    }
  }
  ...
  if (typeof payload === "object") {
    ...
    for (const key of ["parameters", "arguments", "args", "tool_input", "toolInput", "input", "toolCall", "tool_call"]) {
      ...
      if (val && typeof val === "object") {
        files.push(...extractTargetFiles(val));
...
function formatGuardStdout(result, payload = {}, env = process.env, cliArgs = {}, rawStdin = "") {
  ...
  if (isAntigravity) {
    ...
    if (isBlocked) {
      const reason = [
        result.reason || "Scope boundary violation",
...
async function main(argv = process.argv.slice(2)) {
  ...
  const result = evaluateScopeBoundaryEnforcer(currentPayload, {
    projectRoot: currentCliArgs.project || process.cwd(),

...
A Codex skill using the default invocation policy. Its required **description** is available for skill
selection, so Codex may choose it when the user's task matches; the user can also invoke it explicitly
as `$skill-name`. It spends **context load** in exchange for automatic discovery. In
`agents/openai.yaml`, omission of `policy.allow_implicit_invocation` or a value of `true` has this
meaning.
...
A Codex skill that keeps its required **description** but declares
`policy.allow_implicit_invocation: false` in `agents/openai.yaml`. Codex does not select it implicitly
from an ordinary prompt, while explicit `$skill-name` invocation remains available. It trades
automatic discovery for **cognitive load**: the user must remember when to reach for it.

_Avoid_: procedure, workflow, command

### Description

Required frontmatter metadata that identifies what the skill does and when it applies. For an
**implicitly discoverable** skill it is the top-level **context pointer** used during selection. For an
**explicit-only** skill it remains available to selectors and UI even though policy disables implicit
selection. Presence of the description is not the invocation-policy switch.
...
The token and attention cost of making a skill **implicitly discoverable**. Concise descriptions
reduce that cost and keep skill selection discriminating. **Explicit-only** policy avoids automatic
injection without deleting required metadata.

_Avoid_: token cost, context bloat

### Cognitive Load

The cost an **explicit-only** skill imposes on the human — what they must hold in their head: which
skills exist and when to invoke each one. Automatic discovery removes some of this burden. Cognitive
load is sometimes intentional when human judgment should choose the workflow.

_Avoid_: human index, burden, overhead

### Router Skill

An **explicit-only** entrypoint whose job is to name related explicit-only skills and explain when the
user should invoke each one. It reduces **cognitive load** without changing those skills' invocation
policies.
...
How finely you divide skills. Finer division spends one of two loads: more **implicitly discoverable**
skills increase **context load** and routing competition; more **explicit-only** skills increase
**cognitive load**. Two cuts guide the division. By discovery, split off an implicitly discoverable
skill where a distinct **leading word** should trigger it. By sequence, split a run of **steps** where
visible **post-completion steps** tempt the agent to rush the current one. Beware the reverse: merging
sequences exposes later steps and can invite premature completion.
...
independent of invocation policy: a skill can be implicitly discoverable or explicit-only whether it
is all steps, all reference, or both. When a skill has steps, in-file reference that should be
disclosed buries them and turns attending to them into a coin-flip — a variance lever, not just a
...
**Reference** that lives outside the skill system — a plain file, no **description**, no **steps**, not
invocable — that any skill can point at. It is a neutral shared home for material that does not need
independent skill discovery.
...
slips to being done rather than to the work. A between-steps failure: it needs **steps** to occur — a
skill with no steps that quits early instead has thin **legwork** under an unmet demand. A tug-of-war
between visible **post-completion steps** and the **completion criterion**'s clarity causes the rush.
Sharpen the bound first. Only when the criterion is irreducibly fuzzy and the rush is observed should
later steps move behind a real context boundary, such as a separate task or subagent dispatch;
merely naming another skill inline does not clear visible steps.

...
## Companion hooks and safety guards

When an Antigravity skill bundles safety guards or tool interceptors, it declares them in `hooks.json`
and places handlers under `scripts/`. All handlers executed during `PreToolUse` MUST strictly comply
with the C++ Protobuf contract (`{ decision: "allow" | "deny", reason?: string }`) with zero top-level
schema pollution. For full schema details and packaging rules, read [companion-hooks.md](companion-hooks.md).
...
- For companion hooks, guards, and strict Protojson contracts, read
  [companion-hooks.md](companion-hooks.md).

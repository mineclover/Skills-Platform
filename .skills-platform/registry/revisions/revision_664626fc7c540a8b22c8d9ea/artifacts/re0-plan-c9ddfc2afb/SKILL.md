...
- **Docs stay aligned, like `re0-release` at ship time.** When the cycle's change touches shippable surface, its quality gates always include an **AGENTS.md/README reflection gate**, assigning each reflection to its register per AGENTS.md's docs-role split: internal mechanism/rules/anatomy → **AGENTS.md** (LLM + human, injected as instructions), user-facing catalog/experience → **README** (human-only, never injected). Never dump agent-instruction mechanism into README, and never skip README for a user-facing change. This is the plan-time complement to `re0-release`'s ship-time README step.
